...
export async function GET(request) {
  ...
  if (!ip) {
    console.error("[AbuseIPDB Next.js API Error] Missing IP parameter in query string.");
  ...
  try {
    ...
    if (!apiKey || !apiKey.trim()) {
      ...
      console.error(`[AbuseIPDB Next.js API Error] ${errMessage}`);
      return NextResponse.json({ error: errMessage, apiKeyStatus: "Missing" }, { status: 500 });
    }

    console.log(`[AbuseIPDB Next.js API] Querying live AbuseIPDB external API for ${cleanIp}...`);
    ...
    if (res.ok) {
      ...
      console.log(`[AbuseIPDB Next.js API Success] ${cleanIp} -> Score: ${score}%, Status: ${riskStatus}`);
      ...
      console.error(`[AbuseIPDB Next.js API HTTP ${res.status}] External fetch error for ${cleanIp}: ${errText}`);
      return NextResponse.json({ 
    ...
    console.error(`[AbuseIPDB Next.js API Exception] Network/CORS/Fetch error querying ${cleanIp}:`, error);
    return NextResponse.json({ 
